...
import json # Thêm import cho JSON
...
from pyspark.sql.functions import (
    ...
    to_date, datediff, udf # Thêm udf
)
from pyspark.sql.types import (
    DoubleType, StringType, StructType, StructField,
    IntegerType, TimestampType, DateType, BooleanType # Thêm BooleanType
)
from pyspark.sql.window import Window # Cần cho các hàm cửa sổ như lag, z-score
...
Z_SCORE_THRESHOLD = 2.0 # Có thể điều chỉnh, hoặc làm species-specific hơn nữa

# --- Định nghĩa các hằng số và ngưỡng riêng cho CHÓ ---
DOG_NORMAL_FOOD_AVG = 250.0  # Lượng thức ăn trung bình (g) cho chó
DOG_NORMAL_FOOD_STD = 50.0
DOG_NORMAL_SLEEP_AVG = 12.0 # Giờ ngủ trung bình cho chó
DOG_NORMAL_SLEEP_STD = 2.0
DOG_NORMAL_WEIGHT_AVG = 15.0 # Kg, ví dụ cho giống chó trung bình
DOG_NORMAL_WEIGHT_STD = 5.0

DOG_FOOD_ANOMALY_SCORE = 1
DOG_SLEEP_ANOMALY_SCORE = 1
DOG_WEIGHT_ANOMALY_SCORE = 2
# Meow_count không áp dụng hoặc có điểm 0 cho chó
DOG_MEOW_ANOMALY_SCORE = 0

DOG_NOTE_STRESS_SCORE = 2
DOG_NOTE_ILLNESS_SCORE = 3

DOG_HEALTH_RED_THRESHOLD = 5 # Ngưỡng Risk Score Đỏ
DOG_HEALTH_ORANGE_THRESHOLD = 3 # Ngưỡng Risk Score Cam

# --- Định nghĩa các hằng số và ngưỡng riêng cho MÈO ---
CAT_NORMAL_FOOD_AVG = 180.0  # Lượng thức ăn trung bình (g) cho mèo
CAT_NORMAL_FOOD_STD = 30.0
CAT_NORMAL_SLEEP_AVG = 15.0 # Giờ ngủ trung bình cho mèo
CAT_NORMAL_SLEEP_STD = 3.0
CAT_NORMAL_WEIGHT_AVG = 4.0 # Kg, ví dụ cho giống mèo trung bình
CAT_NORMAL_WEIGHT_STD = 1.0

CAT_NORMAL_MEOW_AVG = 5.0 # Số lần meo trung bình cho mèo
CAT_NORMAL_MEOW_STD = 3.0

CAT_FOOD_ANOMALY_SCORE = 1
CAT_SLEEP_ANOMALY_SCORE = 1
CAT_WEIGHT_ANOMALY_SCORE = 2
CAT_MEOW_ANOMALY_SCORE = 1 # Meow_count áp dụng cho mèo

CAT_NOTE_STRESS_SCORE = 2
CAT_NOTE_ILLNESS_SCORE = 3

CAT_HEALTH_RED_THRESHOLD = 5 # Ngưỡng Risk Score Đỏ
CAT_HEALTH_ORANGE_THRESHOLD = 3 # Ngưỡng Risk Score Cam
...
# Hàm UDF để kiểm tra từ khóa trong ghi chú (không phân biệt hoa thường)
...
# UDF sẽ được định nghĩa lại trong hàm phân tích để sử dụng từ khóa đúng loài
...
# Xử lý các giá trị NULL/NaN trong các cột số quan trọng
...
# Dữ liệu pet_profiles (được nhúng trực tiếp vào script)
pet_profiles_raw = [
...
# Join df_raw_data với df_pet_profiles để có cột 'species' và 'name'
df_with_profiles = df_raw_data.join(
    df_pet_profiles.select("pet_id", "species", "name", "breed"), # Lấy thêm breed cho tiện
    on="pet_id",
    how="left"
)
...
owner_notes_path = hdfs_base_path + "owner_notes.md"
...
try:
    ...
    print(f"Đã đọc {df_notes.count()} bản ghi từ owner_notes.md.")
    df_notes.show(5, truncate=False)

except Exception as e:
    print(f"LỖI: Không thể đọc hoặc xử lý owner_notes.md '{owner_notes_path}'. Lỗi: {e}")
    # Tiếp tục chạy mà không có notes nếu lỗi, hoặc dừng tùy mức độ nghiêm trọng
...
# Join df_with_profiles với df_notes
...
# Xử lý các giá trị null cho cột note_text nếu không có ghi chú cho ngày đó
...
def analyze_species_data(df_species, species_type):
    ...
    # Chọn bộ hằng số và từ khóa phù hợp
    if species_type == "dog":
        ...
        z_score_threshold = Z_SCORE_THRESHOLD # Có thể điều chỉnh riêng ở đây nếu muốn
        ...
        meow_anomaly_score = DOG_MEOW_ANOMALY_SCORE # Sẽ là 0
        ...
        has_meow_count = False # Cờ để biết có áp dụng meow_count không

    else: # species_type == "cat"
        ...
        note_illness_score = CAT_ILLNESS_SCORE
        ...
        has_meow_count = True # Cờ để biết có áp dụng meow_count không

    # Định nghĩa cửa sổ để tính toán trung bình và độ lệch chuẩn cho từng pet
    window_spec = Window.partitionBy("pet_id").orderBy("date")

    # 1. Tính toán Z-score và phát hiện bất thường
    ...
    # 3. Tính Risk Score tổng hợp (bao gồm cả note_text)
    df_analyzed = df_analyzed.withColumn("risk_score", lit(0).cast(IntegerType())) # Khởi tạo
    ...
    else: # Nếu là chó, đảm bảo meow_anomaly_score không ảnh hưởng
        df_analyzed = df_analyzed.withColumn("risk_score", col("risk_score"))

    # Cộng điểm từ ghi chú chủ nuôi
    df_analyzed = df_analyzed.withColumn(
        "risk_score",
...
# Kết hợp lại các DataFrame đã phân tích của chó và mèo
df_final_report = df_dogs_analyzed.unionAll(df_cats_analyzed)

# Lọc chỉ những bản ghi có Risk Score > 0 để tạo báo cáo cảnh báo
...
output_alerts_path = output_path + "pet_health_alerts_final.csv" # Đổi tên file để tránh ghi đè
